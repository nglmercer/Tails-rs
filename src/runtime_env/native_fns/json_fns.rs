...
use super::helpers::{to_string_value, to_json_value, from_json_value};

pub(super) fn native_json_parse(interp: &mut Interpreter, _this: &Value, args: &[Value]) -> Result<Value> {
...
pub(super) fn native_json_stringify(interp: &mut Interpreter, _this: &Value, args: &[Value]) -> Result<Value> {
